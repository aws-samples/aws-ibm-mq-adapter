...
export class IBMMQMockStack extends Stack {
  ...
  constructor(scope: Construct, id: string, props: MQAdapterProps) {
    ...
    const fargateCluster = new aws_ecs.Cluster(this, 'ibmMQFargatCluster', { vpc });
    let ibmmq = null;

    const mqAdapterIbmMockClientPublicCertARN = this.node.tryGetContext('stages')[props.stage].mock.IBM_MOCK_PUBLIC_CERT_ARN;
    const mqAdapterIbmMockPrivateCertARN = this.node.tryGetContext('stages')[props.stage].mock.IBM_MOCK_PRIVATE_CERT_ARN;
    const mqAdapterIbmMockPublicCertARN = this.node.tryGetContext('stages')[props.stage].mock.IBM_MOCK_CLIENT_PUBLIC_CERT_ARN;
